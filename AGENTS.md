...
- **Oxlint config** in `vite.config.ts:122-196`: plugins `react, typescript, jsx-a11y, oxc` + 2 jsPlugins (`vite-plus/oxlint-plugin`, `anti-slop` → `./tools/oxlint/anti-slop/index.ts`). Ignores **14 patterns** (`dist/**`, `lib/cloudflare-env.d.ts`, `.agent/**`, `.agents/**`, `.claude/**`, `.codex/**`, `.continue/**`, `.cursor/**`, `.gemini/**`, `.opencode/**`, `.pi/**`, `.roo/**`, `.windsurf/**`, `tools/oxlint/anti-slop/**`) shared by `lint`+`fmt`. Rules: **18 `anti-slop/*` at error** (all generic rules) + native `oxc/no-accumulating-spread`; `@oxlint/plugins` pinned to the `vite-plus` oxlint version; provenance/rules/limits in `tools/oxlint/anti-slop/UPSTREAM.md`; `tsconfig.json` excludes `tools` (vendored, not application source). Overrides for `__tests__/**` etc. Staged hook: `staged: {"*.{ts,tsx,js,jsx,json,css}": ["vp check --fix"]}` (Vite+ native, not husky).
