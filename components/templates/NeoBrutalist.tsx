      ...
      <div className="max-w-6xl mx-auto space-y-8 pb-20">
        ...
        <nav className="flex justify-between items-center bg-white border-4 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="hidden md:flex gap-6 font-bold text-sm uppercase">
            {content.experience && content.experience.length > 0 && (
              <a href="#experience" className="hover:bg-[#FF90E8] px-2 transition-colors">Experience</a>
            )}
            {content.projects && content.projects.length > 0 && (
              <a href="#work" className="hover:bg-[#22CCEE] px-2 transition-colors">Work</a>
            )}
            {content.education && content.education.length > 0 && (
              <a href="#education" className="hover:bg-[#FFDE00] px-2 transition-colors">Education</a>
            )}
          </div>
        </nav>
        ...
        <header className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 bg-[#FF90E8] border-4 border-black p-8 md:p-16 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden group">
            ...
            <h1 className="text-5xl md:text-7xl font-black uppercase leading-[0.85] tracking-tighter relative z-10">
              {content.headline.split(' ').slice(0, 2).join(' ')}<br/>
              {content.headline.split(' ').slice(2).join(' ')}<br/>
              <span className="text-white" style={{WebkitTextStroke: '2px black'}}>Portfolio</span>
            </h1>
            {content.summary && (
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            <div className="bg-white border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
              <h3 className="font-black text-lg uppercase mb-4 underline decoration-4 decoration-[#FFDE00]">Connect</h3>
              <div className="grid grid-cols-2 gap-3">
                {content.contact.email && (
                  <a href={`mailto:${content.contact.email}`} className="flex items-center justify-center p-3 border-2 border-black bg-white hover:bg-black hover:text-white transition-colors font-bold text-xs uppercase gap-2">
                    Email
                  </a>
                )}
                {content.contact.linkedin && (
                  <a href={content.contact.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center p-3 border-2 border-black bg-white hover:bg-black hover:text-white transition-colors font-bold text-xs uppercase gap-2">
                    LinkedIn
                  </a>
                )}
                {content.contact.github && (
                  <a href={content.contact.github} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center p-3 border-2 border-black bg-white hover:bg-black hover:text-white transition-colors font-bold text-xs uppercase gap-2">
                    GitHub
                  </a>
                )}
                {content.contact.website && (
                  <a href={content.contact.website} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center p-3 border-2 border-black bg-white hover:bg-black hover:text-white transition-colors font-bold text-xs uppercase gap-2">
                    Website
                  </a>
                )}
              </div>
        ...
        {content.experience && content.experience.length > 0 && (
          <div id="experience" className="space-y-6 mb-16">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h2 className="text-4xl font-black uppercase bg-white border-4 border-black px-6 py-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transform rotate-1">Experience</h2>
              <div className="h-1 bg-black flex-1"></div>
            </div>
              ...
              {content.experience.map((job, _idx: number) => (
                ...
                >
                  <div className="border-b-4 border-black p-3 flex justify-between items-center bg-neutral-100">
                    ...
                    <span className="font-bold text-xs uppercase">{formatDateRange(job.start_date, job.end_date)}</span>
                  </div>
        ...
        {content.projects && content.projects.length > 0 && (
          <div id="work" className="space-y-6">
            <div className="flex items-center gap-4 mb-8">
              <div className="h-1 bg-black flex-1"></div>
              <h2 className="text-4xl font-black uppercase bg-white border-4 border-black px-6 py-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transform rotate-1">Selected Projects</h2>
              <div className="h-1 bg-black flex-1"></div>
            </div>
              ...
              {content.projects.map((project: Project, _idx: number) => (
                ...
                >
                  <div className="border-b-4 border-black p-3 flex justify-between items-center bg-neutral-100">
                    ...
                    {project.url && (
                      <a href={project.url} target="_blank" rel="noopener noreferrer" className="font-bold text-xs uppercase hover:underline">
                        View →
                      </a>
                    )}
                  </div>
        ...
        {content.education && content.education.length > 0 && (
          <div id="education" className="space-y-6 my-16">
            <div className="flex items-center gap-4 mb-8">
              <div className="h-1 bg-black flex-1"></div>
              <h2 className="text-4xl font-black uppercase bg-white border-4 border-black px-6 py-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transform -rotate-1">Education</h2>
              <div className="h-1 bg-black flex-1"></div>
            </div>
              ...
              {content.education.map((edu, index) => (
                <div key={index} className="bg-white border-4 border-black p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    {edu.graduation_date && (
                      <span className="font-bold text-xs uppercase">{new Date(edu.graduation_date).getFullYear()}</span>
                    )}
                  </div>
                  <h3 className="text-2xl font-black uppercase mb-2">{edu.degree}</h3>
                  <p className="font-bold text-sm">{edu.institution}</p>
                  {edu.gpa && (
                    <p className="text-xs font-bold mt-2 bg-green-300 border-2 border-black inline-block px-2 py-1">GPA: {edu.gpa}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {content.certifications && content.certifications.length > 0 && (
          <div className="space-y-6 my-16">
            <div className="flex items-center gap-4 mb-8">
              <div className="h-1 bg-black flex-1"></div>
              <h2 className="text-4xl font-black uppercase bg-white border-4 border-black px-6 py-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">Certifications</h2>
              <div className="h-1 bg-black flex-1"></div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {content.certifications.map((cert, index) => (
                <div key={index} className="bg-white border-4 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {cert.url && (
                        <a href={cert.url} target="_blank" rel="noopener noreferrer" className="text-xs font-bold underline mt-1 inline-block">
                          View →
                        </a>
                      )}
                    </div>
