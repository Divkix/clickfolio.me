...
function NeoBrutalistNav({ content }: { content: TemplateProps["content"] }) {
  return (
    <nav
...
function NeoBrutalistHero({ content, profile }: TemplateProps) {
  const projectsHook =
    content.projects && content.projects.length > 0
...
function NeoBrutalistMarquee({ skills }: { skills: string[] }) {
...
function SectionHeading({ title, tilt }: { title: string; tilt?: "left" | "right" }) {
  ...
  return (
    <div className="flex items-center gap-4 mb-8">
      ...
      <h2
        className={`font-heading-nb text-3xl md:text-4xl font-black uppercase bg-white border-2 md:border-4 border-black px-6 py-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]${rotation} cursor-default shrink-0`}
      >
        {title}
      </h2>
      <div className="h-1 bg-black flex-1" />
    ...
    <>
      ...
      <div className="min-h-screen bg-[#FFFDF5] text-black font-mono p-4 md:p-6 overflow-x-hidden selection:bg-[#FF90E8] selection:text-black">
        <main className="max-w-6xl mx-auto space-y-8 pb-12">
          <NeoBrutalistNav content={content} />

          <NeoBrutalistHero content={content} profile={profile} />

          <NeoBrutalistMarquee skills={flatSkills} />
