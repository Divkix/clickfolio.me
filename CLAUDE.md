...
app/                        # vinext App Router
  ...
  blog/                     # Static blog pages — 17 hardcoded route folders, not DB-driven — ISR 86400 (list page too)
                            ...
                            #   layout.tsx: SiteHeader + Footer (same public nav as homepage); listing/posts keep their own <main>
...
lib/
  ...
  cron/                     # 3 scheduled task implementations
  ...
  seo/                      # JSON-LD + per-route OG/Twitter (`page-metadata.ts`) + sitemap helpers
...
- `__tests__/` (root-level `*.test.ts`) — must be **hard-coded** into the right config's `include` array or it runs ONLY in combined coverage. Current assignment (all 10 assigned, none in two suites):
  - **Unit:** privacy, profile-schema, resume-schema, sitemap, theme-id-consistency
  - **Integration:** claim-flow, share
  - **Security:** idor-ownership, sanitization, password-strength, email-verification, claim-security-cookie
- `__tests__/e2e/` — has an `e2e/fixtures/` subdir but no active `.test` files; excluded from every run.

**Dominant test pattern — mock-then-dynamic-import:** declare top-level `vi.mock("<module>", () => ({...}))` (hoisted), then inside `it()` do `const { POST } = await import("@/app/api/.../route")` so the SUT loads AFTER the mocks. `vi.doMock` (used in upload-claim-parse.test.ts) is NOT hoisted, for per-test dynamic mocking. **Inline hand-rolled mocks are the norm** — only ~9 of 96 files import the shared `@/__tests__/setup/...` fixtures; match the inline style of the suite you're editing.
...
- `posthog-node` is globally mocked in `__tests__/setup.ts` so API tests never send analytics over the network; `analytics-server.test.ts` overrides that mock to verify `lib/analytics/server.ts` directly.
...
- **3 cron triggers** called **directly** (not via HTTP self-fetch — ADR-0013). Dispatched by `controller.cron` in `scheduled()`. Each case early-returns with an error log if its required binding is missing (R2 for `0 2`, Queue for `*/15`); the whole switch is wrapped in try/catch (a throwing cron does NOT crash the worker); unknown expressions log `unknown cron trigger`.
...
| ------------------------ | ---------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `HYPERDRIVE`             | Hyperdrive | PlanetScale Postgres     | `id 8132893bf32b4e0b8b1b7edc8dad16c1` → logical DB `clickfolio`. Use via `getDb(env.HYPERDRIVE)` only. The legacy D1 database `clickfolio-db` is kept REMOTELY as a dormant backup — no binding, no code path |
...
- Optional: `ALERT_CHANNEL` (`logpush | webhook`, default `logpush`), `ALERT_WEBHOOK_URL` (Slack/Discord-compatible on permanent queue failures), `DISABLE_RATE_LIMITS` (`'true'` bypasses limits — IGNORED in production), `MAX_UPLOAD_SIZE_MB` (default 5), `RATE_LIMIT_UPLOADS_PER_DAY` (default 5), Umami vars (`NEXT_PUBLIC_UMAMI_WEBSITE_ID`, `UMAMI_API_URL`, `UMAMI_USERNAME`, `UMAMI_PASSWORD`). PostHog needs NO Worker vars: the public token + managed proxy host (`https://s.clickfolio.me`) are literals in `lib/analytics/config.ts`. Source-map upload credentials (`POSTHOG_API_KEY`, `POSTHOG_PROJECT_ID`) live ONLY in gitignored local deploy env and are read at build time when `POSTHOG_UPLOAD_SOURCEMAPS=true` (set transiently by `scripts/deploy.ts`; dry-runs force it to `"false"`).
...
- `GET /api/cron/*` — manual cron triggers (`cleanup`, `cleanup-r2`, `recover-orphaned`); `Bearer ${CRON_SECRET}` (`requireCronAuth`, fail-closed).
...
- `GET /api/og/home` — branded 1200×630 **PNG** (`Content-Type: image/png`, `max-age=604800`, no DB/auth/params). The card is still authored as SVG then rasterized with `@cf-wasm/resvg/workerd` (same `Resvg.async(svg,{fitTo:{mode:'width',value:1200}}).render().asPng()` path as handle OG). Facebook/LinkedIn/Slack drop SVG OG images, so this must stay a raster type; a resvg failure returns 500 (no SVG fallback).
...
- **Analytics** — Dual stack: **Umami** (pageviews/dashboard charts) + **PostHog** (product events + identify + error tracking). Typed boundary in `lib/analytics/` (`config.ts` public token + managed proxy `https://s.clickfolio.me` + US UI host, `events.ts` `AnalyticsEventMap`, `client.ts`, `server.ts`). Client: `instrumentation-client.ts` imports `posthog-js/dist/module.no-external` (+ `dist/exception-autocapture` preload), inits once with `defaults:"2026-05-30"`, `capture_exceptions:true`; all product surfaces call `trackAnalyticsEvent`/`identifyAnalyticsUser`/`resetAnalyticsIdentity` from `lib/analytics/client.ts` — never import posthog-js directly; `PostHogIdentifier` in root layout identifies with `externalId ?? id`. Server: `captureServerEvent` (fire-and-forget via `waitUntil`, fresh client per send, `flushAt:1`, no retries) and `captureServerException` (returns the promise) in `lib/analytics/server.ts`; root `instrumentation.ts` forwards `onRequestError` path/method/route metadata (never headers). Source maps upload only when `POSTHOG_UPLOAD_SOURCEMAPS=true` (`vite.config.ts` gates `@posthog/rollup-plugin`; `scripts/deploy.ts` sets it for real deploys, `"false"` on dry-runs).
...
- **4 `error.tsx` levels:** `app/error.tsx`, `app/global-error.tsx` (own html/body, **hardcoded hex colors** since globals.css isn't loaded), `app/[handle]/error.tsx`, `app/(protected)/error.tsx`. ALL FOUR log locally (`console.error`) and report through `captureAnalyticsError(error)` from `lib/analytics/client.ts` (fire-and-forget; `$exception` bypasses the owner-suppression `before_send`). The old `/api/client-error` POST route was removed.
...
- **Root layout** (`app/layout.tsx`): wraps everything in `<ClerkProvider publishableKey={env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY}>` (runtime `env`, not build-time inlining); injects the `umamiBeforeSend` owner-suppression script; loads Umami from `https://analytics.divkix.me/script.js`; `metadataBase=siteConfig.url`, title template `%s | <fullName>`, `manifest=/manifest.webmanifest`, `openGraph.siteName` only (NO default `images`/`url`/`type` — inner pages must set their own via `buildPublicPageMetadata` in `lib/seo/page-metadata.ts`); viewport themeColor light `#fbfaf9`/dark `#121211`; skip-link to `#main-content`.
...
**Visual QA conventions (all 10 templates):** job highlights render as lists, never chips (long text overflows chips); template CSS is scoped to a root class — never `:root`/`html` from a template (`GlassMorphic` uses `.glass-morphic-root`); display headings set `[text-wrap:unset]` so they are not warped by the global `h1–h4 { text-wrap: balance }`; missing project/cert URLs wrap in `<article>`, not `<a href="#">` / `pointer-events-none`; dates go through `formatYear`/`formatShortDate` (never raw ISO). `DevTerminal` tab active state is CSS-only (`:has(#id:target)` on `.term-root`; README is the default when no hash). Preview at `/preview/<themeId>` (demo data, no auth). Shared utilities in `app/globals.css`: `.no-scrollbar`, `.mask-image-b`.
...
| [0019](docs/adr/0019-disposable-email-fail-open.md)              | **SUPERSEDED (Clerk):** disposable-email check was fail-open (availability over strictness)          |
...
**JSON-LD generators** live in `lib/seo/json-ld.ts` (Person/ProfilePage, the homepage trio WebSite+Organization+SoftwareApplication, CollectionPage explore, FAQPage, `generateWebPageJsonLd`, BlogPosting/`generateBlogListingJsonLd`, and breadcrumb helpers). Visible crumbs on `/explore`, `/blog` (listing + posts), privacy, and terms emit BreadcrumbList from `components/ui/breadcrumb.tsx` via `generateBreadcrumbListJsonLd` (do not also emit a page-level BreadcrumbList there — that would duplicate the graph). Profile pages pass `includeJsonLd={false}` and keep `generateBreadcrumbJsonLd` (Home > Explore > displayName). Article schema is not used; posts emit `BlogPosting`. **`serializeJsonLd` (json-ld.ts) is mandatory** before embedding JSON-LD in `dangerouslySetInnerHTML` (escapes `< >` → `</>` to prevent `</script>` breakout, plus U+2028/U+2029). Never `JSON.stringify` JSON-LD directly into a script tag.

**Per-route Open Graph / Twitter** (`lib/seo/page-metadata.ts` `buildPublicPageMetadata` + `HOME_OG_IMAGE`): every public marketing page must set `openGraph.{title,description,type,url,siteName,images}` and `twitter.{card:summary_large_image,title,description,images}` itself. Root layout must not supply a default OG image — Next.js merges nested metadata, so a thin About `twitter:card=summary` would otherwise inherit the homepage 1200×630 PNG. Homepage H1 includes the query **resume website builder** (document title is already `Free Resume Website Builder…`). About document title describes the page (`About — from PDF resume to hosted portfolio`); the root template `%s | clickfolio.me` supplies the brand — do not put `clickfolio.me` in the page title string (OG/Twitter use a branded `ogTitle` because they skip that template). Blog listing H1 is `Writing about resume websites` (not the generic word `Blog`). OG image URL is always `/api/og/home` (PNG) except profile pages (`/api/og/[handle]`).
...
- **Breadcrumb generators:** `generateBreadcrumbListJsonLd(items)` is the generic builder (Home path `/` → `siteConfig.url` with no trailing slash). `generatePageBreadcrumbJsonLd(name,path)` = 2-item Home>Page (for/ pages, FAQ, About — pages without the visible `Breadcrumb` nav). `generateBreadcrumbJsonLd(handle,name)` = 3-item Home>Explore>displayName (profile only). Visible `Breadcrumb` on explore/blog/privacy/terms emits the matching BreadcrumbList itself.
...
**Blog (2-file pattern, both required, kept in sync):** (1) add a `BlogPostMeta` entry to `BLOG_POSTS` in `lib/blog/posts.ts` (`slug, title, description, date[ISO], dateModified?, readTime, category, keywords[], faq?[{q,a}]`); (2) create `app/blog/<slug>/page.tsx` doing `const post = getPostBySlug("<slug>")!` at MODULE scope (non-null — a folder-name vs `BLOG_POSTS` slug desync throws at BUILD time), `revalidate=86400` (same as the LISTING `app/blog/page.tsx`), a module-scope `relatedPosts` = `[slugs].map(getPostBySlug).filter(Boolean) as (typeof post)[]`, `generateMetadata()` via `buildBlogPostMetadata` (canonical `${siteConfig.url}/blog/${slug}`, `og:type=article`, `og:url`, Twitter large-image + PNG `/api/og/home`, robots index+follow), `<BlogPostLayout post={post} relatedPosts={relatedPosts}>`. `app/blog/layout.tsx` wraps listing **and** posts in `SiteHeader` + `Footer` (Explore / Blog / FAQ / About — not a "Back to Home"-only bar). `BlogPostLayout.tsx` injects `generateBlogPostingJsonLd` (`headline`/`datePublished`/`author` from `post` + `authorPersona`, never invented) and FAQPage when `post.faq` is non-empty; BreadcrumbList comes from the visible `Breadcrumb`. Listing emits `generateBlogListingJsonLd` (`Blog` + `blogPost[]` of `BlogPosting` from `BLOG_POSTS`) plus WebPage. ⚠️ `app/blog/clickfolio-templates-showcase/page.tsx` is the ONLY non-static-prose post — it imports LIVE `THEME_METADATA`/`getThemeReferralRequirement` from `theme-ids.ts` and renders theme names/descriptions/referral thresholds inline, so editing `theme-ids.ts` changes this published post's text. `authorPersona` ('The clickfolio Careers Desk', `/about`) is the BlogPosting author + visible byline (brand persona, not a real person — keep credentials honest).

**Role landing pages (`app/for/<slug>/page.tsx`, 6 today):** `revalidate=86400`; `metadata` via `buildRolePageMetadata` → `buildPublicPageMetadata` (canonical `${siteConfig.url}${path}`, `og:type=website`, `og:url`, Twitter large-image + PNG `/api/og/home`); emit `generateWebPageJsonLd` + `generatePageBreadcrumbJsonLd` as two `<script>`s; render `<RoleFaqSection items={faqs} />` (exported from `components/Faq.tsx`, imported as `@/components/Faq` — there is no standalone `RoleFaqSection.tsx`) (which itself emits the FAQPage JSON-LD — so a for/ page has 3 schemas total). **CRITICAL SYNC:** for/ slugs MUST match `lib/config/professions.ts` `PROFESSIONS` (consumed by both homepage grid AND sitemap). `components/Faq.tsx` exports `FaqAccordion` (collapsible, blog + /faq), `RoleFaqSection` (always-expanded + FAQPage JSON-LD, for/ pages), and `FaqItem`.
...
**robots (`app/robots.ts`, MetadataRoute.Robots — not static):** base URL `getPublicSiteUrl()`. `*` allows `/` + `/api/og/` and disallows `/admin/ /dashboard/ /edit/ /preview/ /settings/ /waiting/ /wizard/` (the `*` group does **not** Disallow `/api/` — only `/api/og/` is explicitly Allowed; the old `/reset-password/` + `/verify-email/` entries are gone with their pages). Explicit per-AI-crawler groups (GPTBot, ChatGPT-User, ClaudeBot, PerplexityBot, Google-Extended, GoogleOther) Allow `/, /explore, /blog` **and copy the same Disallow list** — robots.txt named groups do not inherit `*` Disallows, so without the copy those bots could crawl `/admin/` `/dashboard/` `/edit/`. `/for/` and `/blog/*` are intentionally indexable. The `next.config.ts` 308 bare-handle redirect regex (negative-lookahead excluding sitemap/robots.txt/favicon.ico/manifest + reserved prefixes incl. `for|ws` — real top-level routes that must never be 308'd to `/@path`) **must be updated whenever a new top-level reserved route is added**.
