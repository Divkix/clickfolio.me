...
- **Theme / template selection** — see "Resume templates" subsection. **All 10 themes are free** — no referral gating or `isPro` unlock (the referral system was removed; `isThemeUnlocked()` always returns true).
...
2. `THEME_METADATA[id]` (`name/description/category/preview`; `referralsRequired` is vestigial and always 0).
