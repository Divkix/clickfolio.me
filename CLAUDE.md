...
**Signed `pending_upload` cookie (`lib/utils/pending-upload-cookie.ts`):** `COOKIE_NAME='pending_upload'`, `COOKIE_MAX_AGE=86400` (**24h** — aligned with R2 `TEMP_CUTOFF_HOURS=24`, ADR-0024; was 30 min under Issue #89). Format `{tempKey}|{expiresAt_ms}|{base64_hmac}`, HMAC-SHA256 over `{tempKey}|{expiresAt}` via `BETTER_AUTH_SECRET`, CryptoKey cached in a module `Map` keyed by secret (`clearKeyCache()` test-only), constant-time verify. Claim requests from the post-auth flow send `pre_auth:true` (schema `claimRequestSchema`), tagged on `resume_claimed`/`resume_claim_cached` PostHog events.
...
- **Onboarding wizard** (`app/(protected)/wizard/page.tsx`) — `"use client"` state machine (NOT routed steps), self-gates via `useSession()` (redirects `/` if no userId). Named step ids `upload|handle|review|privacy|theme` ordered by `getStepOrder(needsUpload)` (5 if upload needed, else 4). Reads the pending upload from the signed HTTP-only cookie (`GET /api/upload/pending`). Returning-user short-circuit: if `onboardingCompleted===true` it claims any pending upload and redirects straight to `/dashboard`. Guards via `initializingRef`/`hasClaimedRef`. POSTs `/api/wizard/complete`, then opens `<YouAreLiveModal/>` (which renders `<Confetti/>`) — modal has an explicit "Go to Dashboard" CTA; close → `router.push('/dashboard')`. **Step progress is persisted to localStorage** (`lib/utils/wizard-progress.ts`, key `clickfolio:wizard-progress`, 24h TTL) so a returning user resumes where they left off; restored after site_data loads (only to steps past `upload`), cleared on completion / `onboardingCompleted`. Completion failure shows an inline **Retry** button on the theme step (re-POSTs `/api/wizard/complete`); 409 → "handle was just taken" message. Client upload uses `lib/utils/upload.ts` (`uploadWithRetry` — XHR real progress + backoff). Wizard privacy defaults: `show_phone:false, show_address:false, show_in_directory:true, hide_from_search:false`. `components/wizard/index.ts` only re-exports `WizardProgress`; other steps imported directly.
...
**Shared helpers a template MUST use:** `getContactLinks(content.contact)` (`lib/templates/contact-links.ts`, null-checked `ContactLinkDescriptor[]`); `getContactIcon(type,opts)` (`components/templates/shared/ContactIcon.tsx`, returns NULL for `behance`/`dribbble` → render inline 'Be'/'Dr'); `TemplateFontLinks` (`components/templates/shared/TemplateFontLinks.tsx`, the 3-tag Google Fonts pattern); date/skills helpers in `lib/templates/helpers.ts` (`formatDateRange` → 'Mon YYYY — Present', `formatShortDate`, `formatYear`, `flattenSkills`, `getInitials`, all UTC). Footers rendering `new Date().getFullYear()` MUST add `suppressHydrationWarning`. **Email rendering MUST be client-only** to defeat Cloudflare Email Address Obfuscation (which rewrites visible email text → React #418): dedicated email links use `<EmailLink>` (`components/EmailLink.tsx`, full `mailto:` `<a>` hydrated after mount); emails shown as visible `link.label` inside a `getContactLinks` map use `<ObfuscatedText text={link.label}>` for `link.type==="email"` (the `mailto:` href may stay in SSR — CF rewrites the href _attribute_, which React patches silently, no #418). Disable CF Scrape Shield → Email Obfuscation in the dashboard as belt-and-suspenders (ADR-0024). Every template renders `<ShareBar variant=... />` in its footer. (Some templates keep bespoke icon maps intentionally — not drift to "fix".)
...
| [0024](docs/adr/0024-24h-cookie-and-client-only-email.md)        | 24h `pending_upload` cookie (align w/ R2 TTL) + client-only email rendering    |
