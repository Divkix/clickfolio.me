...
- **`lib/config/`** SoT modules: `site.ts` `siteConfig` (name clickfolio, supportEmail support@clickfolio.me, SEO entity graph: `alternateNames`/`sameAs`/`founder` Person = Divanshu Chauhan / divkix.me, **hardcoded `url: https://clickfolio.me`**); `author.ts` `authorPersona` (brand byline 'The clickfolio Careers Desk', url `/about` — NOT a real person); `professions.ts` `PROFESSIONS` (6 slugs: software-engineer/designer/product-manager/marketer/consultant/student) drives BOTH homepage role grid AND sitemap, slugs match `app/for/<slug>`; `faq.ts` `FAQ_ITEMS` (9) feeds FAQ JSON-LD; `lifecycle.ts` (see resume lifecycle below) — `lib/config/retry.ts` is a deprecated re-export shim (kept for backwards compatibility).
