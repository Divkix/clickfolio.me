...
| Framework       | [vinext](https://github.com/cloudflare/vinext) (Vite-based Next.js — NOT standard Next.js) `^0.1.7` (on Next `^16`, React `^19.2`)                   |
| Toolchain       | Vite+ (`vp`) — `vite-plus@0.2.1` direct dep; `vite`/`vitest` are npm-aliases (`@voidzero-dev/vite-plus-core` / `-test`) force-pinned to `0.1.24` via package.json `overrides` (so `vite` resolves to core `0.1.24`, `vitest` to test `0.1.24`, despite the `0.2.1` direct decl) |
