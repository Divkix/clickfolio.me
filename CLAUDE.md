...
app/                        # vinext App Router
  ...
  (public)/                 # empty (verify-email moved to Clerk-managed flows)
...
**7 tables** across `lib/db/schema/`: `user` (`auth.ts`); `resumes` (`resume.ts`); `site_data` (`site.ts`); `handle_changes`, `upload_rate_limits`, `referral_clicks` (`rate-limit.ts`); `pending_r2_deletions` (`maintenance.ts`). Relations in `relations.ts`; barrel `index.ts`. The Better Auth `session`/`account`/`verification` tables are DROPPED (`migrations_pg/0001_drop_better_auth_tables.sql`) — Clerk owns sessions/credentials/OAuth.
...
**`user`** denormalized columns: `showInDirectory` (mirrors privacy JSON; `notNull default(true)`, `user_show_in_directory_idx`), `referralCount` (maintained by PL/pgSQL triggers, not app code; `user_referred_by_idx`), plus `clerkId` UNIQUE, `handle` UNIQUE, `email` UNIQUE, `referralCode` UNIQUE. `privacySettings` is jsonb with default literal `{"show_phone":false,"show_address":false,"hide_from_search":false,"show_in_directory":true}` that MUST stay equal to `DEFAULT_PRIVACY_SETTINGS_JSON` in `lib/utils/privacy.ts` (duplicated as a literal to avoid a circular import). The `PrivacySettings` TS interface (4 snake_case booleans) lives in `auth.ts`. `isAdmin` boolean default false; `isPro` boolean default false; `role`/`roleSource` enum-text columns.

**Triggers (footgun — drizzle snapshots still do NOT track triggers).** `migrations_pg/0002_referral_count_triggers.sql` defines **5 idempotent PL/pgSQL triggers** (`user_referral_count_after_{insert,referred_by_set,referred_by_cleared,referred_by_moved,delete}`, one shared `maintain_referral_count()` function) keeping `user.referral_count` in sync with `user.referred_by`, with a `CASE WHEN referral_count > 0 … ELSE 0` floor. Postgres triggers survive normal ALTER-based rewrites (unlike SQLite), but after ANY drizzle-kit generate that drops/recreates `user`, re-append this file. Recursion terminates after one level because nested firings see `referred_by` unchanged.
...
- `POST /api/resume/update-theme` — validates `THEME_IDS`, `verifyThemeUnlocked`, 404 if no siteData.
...
- `GET /api/profile/me` (returns `{id,name,email,image,handle,headline,privacySettings(parsed),onboardingCompleted,role,roleSource,isAdmin,createdAt,updatedAt}`), `GET /api/site-data`, `GET /api/user/stats` (`{referralCount,isPro}`), `PUT /api/profile/role` (`role`+`roleSource:'user'`) — all use `requireAuthWithUserValidation()` (404 not 401 on missing local row).
...
- `GET /api/admin/{stats,users,resumes,referrals,analytics}` — all gated via the `withAdmin` wrapper (`requireAdminAuthForApi`), **none rate-limited**. `stats`/`analytics` keep an inner `try/catch → 503` on Umami/DB failure; `users`/`resumes`/`referrals` let the wrapper's catch-all handle unexpected throws → 500. `analytics` validates `?period=` ∈ `{7d,30d,90d}`, cache `private, max-age=30, swr=60`. `resumes` `PAGE_SIZE=25`, `?status=` ∈ `{all,completed,processing,queued,failed}`, GROUPS status: `completed`+`waiting_for_cache`→completed, `queued`+`pending_claim`→queued. `users` `PAGE_SIZE=25` + `?search=` ESCAPES LIKE wildcards via `escapeLikePattern()` + raw `LIKE ... ESCAPE '\'` (Drizzle's `like()` omits ESCAPE).
...
- **`lib/referral.ts`** — browser-only `captureReferralCode`/`getStoredReferralCode`/`clearStoredReferralCode` (localStorage). **`lib/referral-server.ts`** — server-only `writeReferral()` sets `referredBy`/`referredAt` via an atomic `WHERE referredBy IS NULL` Postgres UPDATE (first-referral-wins, TOCTOU-safe); resolves either referralCode (uppercased) or handle (lowercased, `@`-stripped); rejects codes over 64 characters. Keep this split: importing the Hyperdrive/Postgres adapter into the browser module pulls Node-only `postgres-js` code into the client bundle.
...
- **Referral system** — every user gets a unique code at signup (**generated by the Clerk webhook** when it inserts the local row for a new identity; failure logs + 500s the webhook so Clerk retries). `ReferralCapture.tsx` reads `?ref=` (must be inside Suspense), `captureReferralCode(ref)` to localStorage + a single `navigator.sendBeacon("/api/referral/track", {code, source:"homepage"})` per load. Claim flows read `getStoredReferralCode()` → send `referral_code` on `/api/resume/claim` → `clearStoredReferralCode()` on success; server `writeReferral()` sets `referredBy` via atomic `WHERE referredBy IS NULL … RETURNING` UPDATE. `user.referralCount` maintained by PL/pgSQL triggers.
...
- **Email verification & password reset** — fully Clerk-managed (verification emails, reset flows, resend cooldowns). There is NO app-side verify-email/reset-password page anymore; `(public)/` is empty and password policy lives in Clerk's dashboard.
...
- **Referral triggers are NOT in drizzle snapshots** — after any drizzle-kit generate that drops/recreates `user`, re-append `migrations_pg/0002_referral_count_triggers.sql` (Postgres triggers survive plain ALTERs, but not a table rebuild).
