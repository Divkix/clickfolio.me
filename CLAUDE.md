...
> **vitest/coverage version coupling (vite-plus).** `vite-plus` bundles its own `vitest@4.1.10`, and `assertCoverageProviderVersionMatch` hard-errors on any `--coverage` run when the installed `@vitest/coverage-v8` differs from that bundled version (e.g. after `pnpm up -L` bumps `catalog.vitest` + `@vitest/coverage-v8` to 4.1.11: "vite-plus bundles vitest@4.1.10, but @vitest/coverage-v8@4.1.11 is installed"). Keep **both** `pnpm-workspace.yaml` `catalog.vitest` and `@vitest/coverage-v8` pinned to the version vite-plus bundles (currently 4.1.10); bump the pair only when a newer vite-plus ships a matching bundled vitest.
