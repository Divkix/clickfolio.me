...
| Framework       | [vinext](https://github.com/cloudflare/vinext) (Vite-based Next.js — NOT standard Next.js) `1.0.0-beta.3` (on Next `^16.2.11`, React `^19.2.7`)           |
| Toolchain       | Vite+ (`vp`) — `vite-plus@^0.2.6`; `vite` aliases `@voidzero-dev/vite-plus-core@^0.2.6`; `vitest` resolves to `4.1.10` through the pnpm catalog/overrides |
...
- `__tests__/` (root-level `*.test.ts`) — must be **hard-coded** into the right config's `include` array or it runs ONLY in combined coverage. Current assignment (all 15 assigned, none in two suites):
  ...
  - **Integration:** claim-flow, referral, share
...
- `GET|POST|DELETE /api/upload/pending` — manages the cookie. POST has the standard body guards (`validateRequestSize` + `readJsonWithLimit`), does `R2.head` to confirm the temp object exists **before signing** (anti-forgery), sets `sameSite:'lax'` (survives OAuth redirect).
...
- `GET /api/resume/status` — poll. `waiting_for_cache` 10-min timeout (`WAITING_FOR_CACHE_TIMEOUT_MS`)→failed; **`pending_claim` surfaced as `processing` (15%)**, `queued` (25%) and `waiting_for_cache` (30%) too; `can_retry` via `canRetryResume()` (mirrors retry-route gates); `parsed_content` only when completed.
- `POST /api/resume/retry` — manual re-queue. **4 gates in order:** (1) `hasExceededMaxAttempts(totalAttempts>=6)`→429; (2) `isPermanentErrorType(lastAttemptError.type)`→400; (3) `status!=='failed'`→400; (4) `retryCount>=2`→429. Status→`queued` before publish; the UPDATE is **TOCTOU-guarded** (`WHERE status='failed' AND retryCount<2`); 0 rows → **409 CONFLICT** (not 500); rollback to failed on publish failure.
- `GET /api/resume/latest-status` — latest resume for the user; maps `pending_claim`/`queued`/`waiting_for_cache` → `processing` (mirrors `/status`); `can_retry` via the same `canRetryResume()` (loads `totalAttempts`+`lastAttemptError` and mirrors it — the two endpoints agree by construction; asserted by a cross-endpoint test in the resume-operations integration suite).
- `PUT /api/resume/update` — `resumeContentSchemaStrict` + `extractPreviewFields` denormalization + `captureBookmark`; **404 NOT_FOUND (not 500) when no site_data row exists** (mirrors update-theme).
- `POST /api/resume/update-theme` — validates `THEME_IDS`, `verifyThemeUnlocked`, 404 if no siteData; `captureBookmark` after write.
- `POST /api/wizard/complete` — canonical onboarding write. Schema built dynamically `buildWizardCompleteSchema([...THEME_IDS])`. `verifyThemeUnlocked` BEFORE writing. **Re-onboarding handle changes enforce the same 3/24h handle_changes rate limit as `PUT /api/profile/handle`** (first-time onboarding exempt) and insert the audit `handleChanges` row in the same `db.batch`. Atomic `db.batch`: updates user (`handle, privacySettings, showInDirectory, onboardingCompleted:true`) + UPSERTs siteData (`onConflictDoUpdate(target: userId)`, `content:'{}'` placeholder filled later by the consumer, `themeId`). UNIQUE→409.
...
- `POST /api/account/delete` — requires `confirmation` to **case-insensitively equal the user's email** (400 else); standard body guards. Order: (1) delete all resume R2 keys via `Promise.allSettled`, **failed deletes recorded in `pendingR2Deletions` BEFORE the DB batch** (GDPR 2 AM retry); (2) `db.batch([delete verification WHERE identifier=email, delete user WHERE id])` (verification has no user FK; deleting user cascades the rest). Returns optional `warnings:[{type:'r2'}]`, then `captureBookmark()` + clears BOTH cookie forms (`Max-Age=0`) **and both `session_data` forms** (better-auth ghost-session fix).
- `GET /api/handle/check?handle=` — **highest-volume** endpoint (~every 500 ms while typing). Deliberate ordering: (1) format validate FIRST (so invalid input never touches D1/limiter; reserved→`{available:false,reason:'reserved'}` — `RESERVED_HANDLES` in `lib/rate-limit/handle-validation.ts` includes `api/_next/static/public` + `xmlrpc`/`adminer` to block scanner-probe handles); (2) IP rate-limit only for valid formats; (3) plain `getDb()` (no session variant); (4) available→return with ZERO auth cost; (5) only if TAKEN resolve `getAuth()`+`getSession()` to distinguish `{isCurrentHandle:true}` vs taken. **Do not reorder.**
...
- **`lib/rate-limit/`** — IP-based limits in D1, **IPs SHA-256 hashed (GDPR)**. Constants in `lib/rate-limit/ip.ts`: `HOURLY_LIMIT=10`, `DAILY_LIMIT=50` (anon upload), `HANDLE_CHECK_HOURLY_LIMIT=100`, `EMAIL_VALIDATE_HOURLY_LIMIT=30`. Authed: `handle_change` 3/24h (`user.ts`), `resume_upload` (`RATE_LIMIT_UPLOADS_PER_DAY || 5`)/24h. Shared `uploadRateLimits` table by `actionType` (`upload` 24h / `handle_check` 1h / `email_validate` 1h); the upload-count query uses conditional SUM aggregation to save a D1 roundtrip. **Enforcement is an atomic conditional INSERT ... SELECT** (`recordRateLimitAction` in ip.ts): the row is inserted only while the in-window count is below the limit, closing the count-then-insert TOCTOU that let two concurrent requests both pass; `changes===0` = a concurrent request won the last slot → deny. **`getClientIP()` falls back to `"unknown"` when neither CF-Connecting-IP nor X-Forwarded-For exists — `"unknown"` is NOT in `LOCAL_IPS`, so header-less requests are hashed+rate-limited as one shared bucket** (never silently allowed). Authed `checkRateLimit` (`user.ts`) computes `resetAt` from the **oldest in-window row + windowMs** (the real rollover time), not a fixed now+window. **Anon IP fails OPEN; authed fails CLOSED.** Bypasses: `NODE_ENV !== 'production'` OR `isLocalEnvironment()` (BETTER_AUTH_URL localhost — this is what disables limits under `pnpm run preview`, which runs as NODE_ENV=production); `DISABLE_RATE_LIMITS === 'true'` is IGNORED with a `[SECURITY]` warn in production.
- **`lib/email/`** — Cloudflare Email Workers (`env.EMAIL`), no external API keys. `createEmailSender(env, baseURL)`; `getFromEmail(appUrl)` = `noreply@<hostname>` (localhost→`noreply@clickfolio.me`), display name `Clickfolio`. **Gotcha: do NOT wrap `resetUrl`/`verificationUrl` in `encodeURI()`** — Better Auth already encodes them; `encodeURI` re-encodes `%`→`%25`. Reset link says 1h, verification 24h. All user values via local `escapeHtml()`. Send errors returned `{success:false,error}`, never thrown. **Logging is PII-free: `getRecipientDomain()` logs only the domain after `lastIndexOf('@')` — the full email is never logged** (matches the SHA-256 IP-hashing posture). `disposable-check.ts` `isDisposableEmail()`: `TRUSTED_DOMAINS` (43-domain allowlist: gmail/outlook/yahoo/icloud/proton/fastmail/zoho/ISPs…) checked first; module-level `cachedDomains` Set with 1h TTL (`_resetCache()` test-only); `extractDomain()` uses `lastIndexOf('@')`; **fails open** on infra error. KV key `disposable-domains`. The email sender module is `lib/email/cloudflare.ts` (not `sender.ts`); it validates URLs via `new URL()` (NOT `encodeURI`).
...
- `@/*` → project root — defined in `tsconfig.json` (`"@/*": ["./*"]`) and in all four vitest configs' `resolve.alias`. The main `vite.config.ts` does NOT alias `@/*` (the build relies on vinext for `@/*` resolution).
...
- **`*/15` (`lib/cron/recover-orphaned.ts`):** recovers THREE stuck states — `pending_claim` (createdAt>5min), `processing` (queuedAt>15min, falls back to createdAt for legacy null queuedAt), `queued` (same 15-min gate). The re-queue UPDATE is **TOCTOU-guarded on the originally-SELECTed status** (`WHERE id=? AND status=?`; 0 rows = the consumer/retry/move-on already handled it → skip) and **does NOT increment `total_attempts`** (the queue consumer increments per actual attempt; an increment here would double-count every recovered resume). Writes status=`queued`+queuedAt BEFORE publishing; on publish failure ROLLS BACK to `pending_claim` (queuedAt=null), guarded on still-`queued`. 10 rows/run/state; skips `totalAttempts>=6`.
...
- **Account deletion / GDPR** (`app/api/account/delete/`) — clears BOTH cookie forms + `session_data` (`Max-Age=0`). IPs stored only as SHA-256 hashes. (See API Routes for the deletion order/contract.)
...
- **Pending-upload claim** uses one handoff store (`FileDropzone.tsx`): the signed HTTP-only cookie set through `setPendingUploadCookie()`. A failed cookie write fails the upload handoff instead of reporting a false success. `FileDropzone` auto-claims via useEffect once `session.user` is present, `await sleep(100)` to let the OAuth Set-Cookie settle, then `router.replace("/dashboard")`. Reused by `DashboardUploadSection`, `ResumeManagementCard`, `MobileStickyUpload`, home page (modal vs inline via `open`/`onOpenChange`).
