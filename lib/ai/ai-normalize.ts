...
function normalizeExperienceItem(value: unknown): Record<string, unknown> {
	...
	if (result.title === undefined) {
		result.title = pickFirstValue(obj, [
			"title",
			"role",
			"position",
			"job_title",
	...
	if (result.company === undefined) {
		result.company = pickFirstValue(obj, [
			"company",
			"employer",
			"organization",
			"org",
	...
	if (result.location === undefined) {
		result.location = pickFirstValue(obj, [
			"location",
			"city",
			"city_state",
			"cityState",
	...
	if (result.start_date === undefined) {
		result.start_date = pickFirstValue(obj, [
			"start_date",
			"startDate",
			"from",
		]);
	...
	if (result.description === undefined) {
		result.description = pickFirstValue(obj, [
			"description",
			"summary",
			"details",
		]);
...
function normalizeEducationItem(value: unknown): Record<string, unknown> {
	...
	if (result.degree === undefined) {
		result.degree = pickFirstValue(obj, [
			"degree",
			"program",
			"field",
			"major",
...
function normalizeCertificationItem(value: unknown): Record<string, unknown> {
	...
	if (result.issuer === undefined) {
		result.issuer = pickFirstValue(obj, [
			"issuer",
			"organization",
			"org",
			"authority",
	...
	if (result.date === undefined) {
		result.date = pickFirstValue(obj, [
			"date",
			"issued",
			"issued_date",
			"issuedDate",
...
function normalizeProjectItem(value: unknown): Record<string, unknown> {
	...
	if (result.year === undefined) {
		result.year = pickFirstValue(obj, [
			"year",
			"date",
			"date_range",
			"dateRange",
	...
	if (result.technologies === undefined) {
		result.technologies = pickFirstValue(obj, [
			"technologies",
			"tech_stack",
			"techStack",
		]);
	...
	if (result.image_url === undefined) {
		result.image_url = pickFirstValue(obj, [
			"image_url",
			"imageUrl",
			"image",
			"thumbnail",
 ...
 */
export function normalizeAiKeys(
	data: Record<string, unknown>,
): Record<string, unknown> {
	const result = { ...data };

	const fullName = pickFirstValue(result, [
		"full_name",
		"fullName",
		"fullname",
		"name",
	...
	if (headline !== undefined && result.headline === undefined)
		result.headline = headline;

	const summary = pickFirstValue(result, ["summary", "profile", "objective"]);
	if (summary !== undefined && result.summary === undefined)
	...
	if (contactSource) {
		...
		if (contact.email === undefined) {
			contact.email = pickFirstValue(contactSource, [
				"email",
				"e-mail",
				"mail",
			]);
	...
	if (experienceArray) {
		result.experience = experienceArray.map((item) =>
	...
	if (educationArray) {
		result.education = educationArray.map((item) =>
	...
	if (certificationsArray) {
		result.certifications = certificationsArray.map((item) =>
			normalizeCertificationItem(item),
		);
	}
	...
	if (professionalLevel !== undefined)
		result.professional_level = professionalLevel;
