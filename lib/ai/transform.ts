...
export function transformAiResponse(raw: unknown): Record<string, unknown> {
	...
	// Top-level fields
	data.full_name = truncateString(
		normalizeString(data.full_name, "Unknown"),
		100,
	);
	data.headline = truncateString(
		normalizeString(data.headline, "Professional"),
		150,
	);
	...
	if (Array.isArray(data.education)) {
		data.education = data.education.filter((edu) => {
			...
			return (
				e.degree && typeof e.degree === "string" && e.degree.trim().length > 0
			);
		});
	...
	if (Array.isArray(data.projects)) {
		...
		for (const proj of data.projects as Record<string, unknown>[]) {
			...
			proj.description = truncateString(
				normalizeString(proj.description),
				1000,
			);
 ...
 */
export function transformAiOutput(
	raw: ResumeContentFormData,
): ResumeContentFormData {
