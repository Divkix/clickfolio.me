...
export async function parseResumeWithAi(
	...
	try {
		...
		let validation = validateParseResult(
			parseResult.data,
		...
		if (!validation.success && validation.errors) {
			console.warn(
				"[ai-parse] Schema validation failed, retrying with error feedback",
				{
		...
		const finalData = transformAiOutput(
			validation.data as ResumeContentFormData,
		);

		// Extract professional_level before serializing — it goes to user.role, not siteData.content
		const professionalLevel = finalData.professional_level as
			| string
			| undefined;
