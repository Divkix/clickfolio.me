...
const ERROR_PATTERNS: Array<{ pattern: RegExp; type: QueueErrorType }> = [
	...
	{
		pattern:
			/D1_ERROR|database.*connection|connection.*refused|SQLITE_BUSY|database.*locked/i,
	...
	{
		pattern:
			/invalid.*pdf|corrupt.*pdf|pdf.*corrupt|pdf.*invalid|malformed.*pdf/i,
	...
	{
		pattern:
			/worker.*not.*available|binding.*not.*available|service.*not.*found/i,
